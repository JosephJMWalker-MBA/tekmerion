...
Future<void> runBenchmark(String name,
  ...
  final agreementBytes =
      Uint8List.fromList(List<int>.filled(agreementMb * 1024 * 1024, 65));
  final agHash = sha256.convert(agreementBytes).toString();

  final numEvidenceFiles = 5;
  final bytesPerEvidence = (evidenceMb * 1024 * 1024) ~/ numEvidenceFiles;
  final evidenceBytes =
  ...
  final exportService = RecordPackageExportService(
    agreementRepo: FakeAgreementRepository([agreement], [version], evidenceMap),
    clauseRepo: FakeClauseRepository([clause1]),
    obligationRepo: FakeObligationRepository([obl1]),
    recordRepo: FakeRecordRepository([rec1]),
    timelineRepo: FakeTimelineRepository(),
    exportRepo: FakeExportPackageRepository(),
    evidenceStorage: FakeEvidenceStorage(fileData),
  ...
  final stream = exportService.generateCompleteExport(agreementId);
...
Future<void> runFailureBenchmark() async {
  print('\n--- Running Failure Benchmark (Controlled Cleanup Test) ---');
  // Inject a failure by mutating one bytes array in Evidence Storage to have different length/hash
  final stopwatch = Stopwatch()..start();

  final agreementBytes = Uint8List.fromList([1, 2, 3]);
  final agHash = sha256.convert(agreementBytes).toString();
  final agreementId = 'fail_test';
  ...
  final agEnv = EvidenceEnvelope(
    ...
    mimeType: 'application/pdf',
    byteSize: agreementBytes.length,
    sha256: agHash, // CORRECT hash in db
    captureMethod: EvidenceCaptureMethod.externalImport,
    ingestedAt: DateTime.now().toUtc(),
    storageIdentifier: 'ag_ev_1',
    assetRole: EvidenceAssetRole.original,
  ...
  // MUTATE DATA to simulate corrupted filesystem or mismatching bytes
  final Map<String, Uint8List> fileData = {
    'ag_ev_1': Uint8List.fromList([1, 2, 3, 4, 5]),
  };
  ...
  print('  Final State: ${finalStatus.state.name}');
  if (finalStatus.state != ExportState.failed) {
    throw Exception('Expected failure but got ${finalStatus.state.name}');
  }

  print('  Result: SUCCESS (Failed closed and cleaned up)');
}
